...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in RKHelperModule.
     *
    ...
    public function getObjectTypes($context = '', array $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'linker';
        $allowedObjectTypes[] = 'carouselItem';
        $allowedObjectTypes[] = 'carousel';
        $allowedObjectTypes[] = 'image';
        $allowedObjectTypes[] = 'info';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in RKHelperModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('RKHelperModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('RKHelperModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('RK\HelperModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkhelpermodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkhelpermodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'linker') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'LinkerImage'] = $this->imageHelper->getRuntimeOptions($objectType, 'linkerImage', $context, $args);
                $parameters['thumbRuntimeOptions'] = $thumbRuntimeOptions;
            }
            if ($objectType == 'carouselItem') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'ItemImage'] = $this->imageHelper->getRuntimeOptions($objectType, 'itemImage', $context, $args);
                $parameters['thumbRuntimeOptions'] = $thumbRuntimeOptions;
            }
            if ($objectType == 'image') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'MyImage'] = $this->imageHelper->getRuntimeOptions($objectType, 'myImage', $context, $args);
                $parameters['thumbRuntimeOptions'] = $thumbRuntimeOptions;
            }
            if ($objectType == 'info') {
                $thumbRuntimeOptions = [];
            ...
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                ...
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'RKHelperModule_relateditem', $context, $args);
            }
        }
