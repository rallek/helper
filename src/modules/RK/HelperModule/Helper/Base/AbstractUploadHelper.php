...
abstract class AbstractUploadHelper
{
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('RKHelperModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('RKHelperModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('RKHelperModule', 'shrinkHeight' . $fieldSuffix, 600);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/RKHelperModule/';
    
        switch ($objectType) {
            case 'linker':
                $basePath .= 'linkers/linkerimage/';
                break;
            case 'carouselItem':
                $basePath .= 'carouselitems/itemimage/';
                break;
            case 'image':
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('linker', 'linkerImage', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'RKHelperModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'RKHelperModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/RK/HelperModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'RKHelperModule', 'directory' => $uploadPath]);
            }
        }
