...
abstract class AbstractLinkerController extends AbstractController
{
     ...
     * @ParamConverter("linker", class="RKHelperModule:LinkerEntity", options = {"repository_method" = "selectById", "mapping": {"id": "id"}, "map_method_signature" = true})
    ...
    protected function deleteInternal(Request $request, LinkerEntity $linker, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKHelperModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'linker', 'id' => $linker->getKey()];
        ...
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $linker);
        $hookHelper = $this->get('rk_helper_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $linker, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($linker, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $linker, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the linker
                    $hookHelper->callProcessHooks($linker, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKHelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'linker', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkhelpermodule_linker_display', $urlArgs);
            }
