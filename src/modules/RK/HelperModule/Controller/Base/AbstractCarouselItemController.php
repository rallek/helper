...
abstract class AbstractCarouselItemController extends AbstractController
{
    ...
    protected function deleteInternal(Request $request, CarouselItemEntity $carouselItem, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKHelperModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'carousel item', 'id' => $carouselItem->getKey()];
        ...
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $carouselItem);
        $hookHelper = $this->get('rk_helper_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $carouselItem, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($carouselItem, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $carouselItem, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the carousel item
                    $hookHelper->callProcessHooks($carouselItem, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function displayInternal(Request $request, CarouselItemEntity $carouselItem, $isAdmin = false)
    {
        ...
        $instanceId = $carouselItem->getKey();
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKHelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'carousel item', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkhelpermodule_carouselitem_display', $urlArgs);
            }
