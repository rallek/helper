...
abstract class AbstractCarouselController extends AbstractController
{
     ...
     * @ParamConverter("carousel", class="RKHelperModule:CarouselEntity", options = {"repository_method" = "selectById", "mapping": {"id": "id"}, "map_method_signature" = true})
     * @Cache(lastModified="carousel.getUpdatedDate()", ETag="'Carousel' ~ carousel.getid() ~ carousel.getUpdatedDate().format('U')")
     *
     * @param Request $request Current request instance
    ...
    protected function deleteInternal(Request $request, CarouselEntity $carousel, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKHelperModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'carousel', 'id' => $carousel->getKey()];
        ...
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $carousel);
        $hookHelper = $this->get('rk_helper_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $carousel, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($carousel, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $carousel, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the carousel
                    $hookHelper->callProcessHooks($carousel, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKHelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'carousel', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkhelpermodule_carousel_display', $urlArgs);
            }
