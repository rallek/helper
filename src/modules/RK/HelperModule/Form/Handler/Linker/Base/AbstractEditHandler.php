...
namespace RK\HelperModule\Form\Handler\Linker\Base;

use RK\HelperModule\Form\Handler\Common\EditHandler;
use RK\HelperModule\Form\Type\LinkerType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'linker';
        $this->objectTypeCapital = 'Linker';
        $this->objectTypeLower = 'linker';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the linker yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(LinkerType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of linker area
        $codes[] = 'userIndex';
        // admin index page of linker area
        $codes[] = 'adminIndex';
        // user list of linkers
        $codes[] = 'userView';
        // admin list of linkers
        $codes[] = 'adminView';
        // user list of own linkers
        $codes[] = 'userOwnView';
        // admin list of own linkers
        $codes[] = 'adminOwnView';
        // user detail page of treated linker
        $codes[] = 'userDisplay';
        // admin detail page of treated linker
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Linker created.');
                } else {
                    $message = $this->__('Done! Linker updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Linker deleted.');
                break;
            default:
                $message = $this->__('Done! Linker updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'linker', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
