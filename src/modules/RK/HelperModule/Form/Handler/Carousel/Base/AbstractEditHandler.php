...
namespace RK\HelperModule\Form\Handler\Carousel\Base;

use RK\HelperModule\Form\Handler\Common\EditHandler;
use RK\HelperModule\Form\Type\CarouselType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'carousel';
        $this->objectTypeCapital = 'Carousel';
        $this->objectTypeLower = 'carousel';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the carousel yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(CarouselType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of carousel area
        $codes[] = 'userIndex';
        // admin index page of carousel area
        $codes[] = 'adminIndex';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Carousel created.');
                } else {
                    $message = $this->__('Done! Carousel updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Carousel deleted.');
                break;
            default:
                $message = $this->__('Done! Carousel updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'carousel', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
