...
namespace RK\HelperModule\Form\Handler\CarouselItem\Base;

use RK\HelperModule\Form\Handler\Common\EditHandler;
use RK\HelperModule\Form\Type\CarouselItemType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'carouselItem';
        $this->objectTypeCapital = 'CarouselItem';
        $this->objectTypeLower = 'carouselitem';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the carousel item yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(CarouselItemType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of carousel item area
        $codes[] = 'userIndex';
        // admin index page of carousel item area
        $codes[] = 'adminIndex';
        // user list of carousel items
        $codes[] = 'userView';
        // admin list of carousel items
        $codes[] = 'adminView';
        // user list of own carousel items
        $codes[] = 'userOwnView';
        // admin list of own carousel items
        $codes[] = 'adminOwnView';
        // user detail page of treated carousel item
        $codes[] = 'userDisplay';
        // admin detail page of treated carousel item
        $codes[] = 'adminDisplay';
    
        // user list of carousells
        $codes[] = 'userViewCarousells';
        // admin list of carousells
        $codes[] = 'adminViewCarousells';
        // user list of own carousells
        $codes[] = 'userOwnViewCarousells';
        // admin list of own carousells
        $codes[] = 'adminOwnViewCarousells';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of carouselItems
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Carousel item created.');
                } else {
                    $message = $this->__('Done! Carousel item updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Carousel item deleted.');
                break;
            default:
                $message = $this->__('Done! Carousel item updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'carousel item', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('rkhelpermodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('rkhelpermodule' . $this->objectTypeCapital . 'Referer');
        }
        ...
        switch ($this->returnTo) {
            ...
            case 'adminDisplay':
                ...
                return $this->getDefaultReturnUrl($args);
            case 'userViewCarousells':
            case 'adminViewCarousells':
                return $this->router->generate('rkhelpermodule_carousel_' . $routeArea . 'view');
            case 'userOwnViewCarousells':
            case 'adminOwnViewCarousells':
                return $this->router->generate('rkhelpermodule_carousel_' . $routeArea . 'view', ['own' => 1]);
            default:
                return $this->getDefaultReturnUrl($args);
        }
    }
