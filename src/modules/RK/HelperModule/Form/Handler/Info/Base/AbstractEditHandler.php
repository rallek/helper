...
namespace RK\HelperModule\Form\Handler\Info\Base;

use RK\HelperModule\Form\Handler\Common\EditHandler;
use RK\HelperModule\Form\Type\InfoType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->objectTypeCapital = 'Info';
        $this->objectTypeLower = 'info';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the info yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(InfoType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of info area
        $codes[] = 'userIndex';
        // admin index page of info area
        $codes[] = 'adminIndex';
        // user list of infos
        $codes[] = 'userView';
        // admin list of infos
        $codes[] = 'adminView';
        // user list of own infos
        $codes[] = 'userOwnView';
        // admin list of own infos
        $codes[] = 'adminOwnView';
        // user detail page of treated info
        $codes[] = 'userDisplay';
        // admin detail page of treated info
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Info created.');
                } else {
                    $message = $this->__('Done! Info updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Info deleted.');
                break;
            default:
                $message = $this->__('Done! Info updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'info', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
