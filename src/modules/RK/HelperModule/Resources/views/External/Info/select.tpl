...
{assign var='baseID' value='info'}
<div id="itemSelectorInfo" class="hidden" data-base-id="{$baseID}" data-selected-id="{$selectedId|default:0}"></div>
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Info'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='info' from=$items}
                        <option value="{$info->getKey()}"{if $selectedId eq $info->getKey()} selected="selected"{/if}>{$info->getInfoTitle()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    <option value="infoTitle"{if $sort eq 'infoTitle'} selected="selected"{/if}>{gt text='Info title'}</option>
                    <option value="infoDescription"{if $sort eq 'infoDescription'} selected="selected"{/if}>{gt text='Info description'}</option>
                    <option value="infoLocale"{if $sort eq 'infoLocale'} selected="selected"{/if}>{gt text='Info locale'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Info information'}</strong></p>
