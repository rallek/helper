...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function contentGetTypes(\Zikula_Event $event)
    {
        ...
        $types->add('RKHelperModule_ContentType_Item');
        
        // plugin for showing a list of multiple items
        $types->add('RKHelperModule_ContentType_ItemList');
    }
     ...
     * RKHelperModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/rkhelper';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/RK/HelperModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/RK/HelperModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                'module' => 'RKHelperModule',
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/rkhelper/js/RKHelperModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkhelpermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkhelper/scribite/CKEditor/rkhelpermodule/',
            'file' => 'plugin.js',
            'img' => 'ed_rkhelpermodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkhelpermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkhelper/scribite/Quill/rkhelpermodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkhelpermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkhelper/scribite/Summernote/rkhelpermodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'rkhelpermodule',
            'path' => $this->request->getBasePath() . '/web/modules/rkhelper/scribite/TinyMce/rkhelpermodule/plugin.js'
        ]);
    }
}
