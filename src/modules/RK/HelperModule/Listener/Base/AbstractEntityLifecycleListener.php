...
use RK\HelperModule\HelperEvents;
use RK\HelperModule\Event\FilterLinkerEvent;
use RK\HelperModule\Event\FilterCarouselItemEvent;
use RK\HelperModule\Event\FilterCarouselEvent;
use RK\HelperModule\Event\FilterImageEvent;
use RK\HelperModule\Event\FilterInfoEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
        
        // delete workflow for this entity
        $workflowHelper = $this->container->get('rk_helper_module.workflow_helper');
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $uploadHelper = $this->container->get('rk_helper_module.upload_helper');
        ...
        $logArgs = ['app' => 'RKHelperModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $objectType, 'id' => $objectId];
        $logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\HelperModule\\Event\\Filter' . ucfirst($objectType) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\HelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $objectId = $entity->createCompositeIdentifier();
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'RKHelperModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $objectId];
        $logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\HelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\HelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\HelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\HelperModule\\HelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'RK' && $entityClassParts[1] == 'HelperModule');
    }
    ...
    protected function getUploadFields($objectType)
    {
        ...
        switch ($objectType) {
            case 'linker':
                $uploadFields = ['linkerImage'];
                break;
            case 'carouselItem':
                $uploadFields = ['itemImage'];
                break;
            case 'image':
